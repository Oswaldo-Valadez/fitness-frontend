...
import { clearConsentRequired, fetchMe } from '@/store/authSlice'
...
export default function OnboardingPage() {
  ...
  const { user, consentRequired } = useAuth()

  // Un usuario que ya tiene perfil llegó aquí solo a reaceptar
  // consentimientos revocados: no tiene sentido repetir el wizard completo.
  const isReaccept = !!user?.has_profile

  const finish = async () => {
    await dispatch(fetchMe())
    dispatch(clearConsentRequired())
    navigate(consentRequired?.returnPath || '/dashboard', { replace: true })
  }
  ...
  const visibleSteps = isReaccept ? STEPS.filter((s) => s.key === 'consents') : STEPS
      ...
      <div className="flex flex-col items-center pt-10">
        ...
        {visibleSteps.length > 1 && (
          <div className="flex items-center gap-3">
            {visibleSteps.map(({ key, label, icon: Icon }, i) => (
              <div key={key} className="flex items-center gap-3">
                ...
                {i < visibleSteps.length - 1 && <div className={clsx('h-0.5 w-10 rounded-full', i < stepIndex ? 'bg-accent' : 'bg-surface-muted')} />}
              </div>
            ))}
          </div>
        )}
